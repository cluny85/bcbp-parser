...
const getUTCDate = (input) => {
  ...
  const dateMatch = input.match(iataJulianDateRegexp);
  if (!dateMatch) throw new Error(`Invalid input, no matching groups found for input (${input})`);
  const { groups: { julianYearLastDigit, julianDayOfYear } } = dateMatch;
